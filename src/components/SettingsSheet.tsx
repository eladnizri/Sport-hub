  ...
  return (
    <div className="sheet-backdrop" onClick={onClose} role="presentation">
      <div className="sheet" onClick={(e) => e.stopPropagation()} role="dialog" aria-label="הגדרות">
        ...
        />
        <Switch
          label="לייב למשחקים שלי"
          hint="מושך עדכון חי ישירות מהספק רק כשרץ משחק של קבוצה ישראלית או לגיונר"
          checked={prefs.liveForMyMatches}
          onChange={(v) => onChange({ liveForMyMatches: v })}
        />
        <Switch
        ...
        <div className="card" style={{ marginTop: 18, padding: 16 }}>
          ...
          <p style={{ fontSize: 12.5, color: 'var(--muted)', margin: '10px 0 0', lineHeight: 1.6 }}>
            רוב הנתונים מגיעים מהקאש שה-GitHub Action בונה, ולכן האפליקציה עובדת
            גם בלי מפתח בדפדפן. <code>VITE_API_FOOTBALL_KEY</code> ב-<code>.env</code>
            מוסיף רק רענון חי בזמן משחק — והוא נארז ל-bundle, אז השאר אותו ריק אם
            האתר ציבורי.
          </p>
        </div>
